...
func fetchAllPostsWithPageNumber(start, count int) (error, []data.Post) {
	...
	for rows.Next() {
		var p data.Post
		if err = rows.Scan(&p.PID, &p.Id, &p.PTitle, &p.Slug, &p.CreatedAt, &p.ModifiedAt, &p.Author, &p.Category, &p.Password, &p.Body); err != nil {
			return err, nil
		}
